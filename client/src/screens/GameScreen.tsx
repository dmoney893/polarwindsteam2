...
export const GameScreen = ({
  ...
  const controlledPlayer = useMemo(() => {
    ...
    const sessionPlayer = room
      ? playerArray.find((p) => p.sessionId === room.sessionId)
      : null;
    if (sessionPlayer) return sessionPlayer;
    return myColor ? playerArray.find((p) => p.color === myColor) ?? null : null;
  }, [room, players, myColor]);

  const aiTeammateLabels = useMemo(() => {
    return Array.from(players.values())
      .filter((p) => p.sessionId.startsWith("solo-"))
      .map((p) => getPlayerDisplayLabel(p.color))
      .join(" + ");
  }, [players]);

  const scoreBarGradient = useMemo(() => {
    const c: PlayerColor = myColor ?? "RED";
    return SCORE_BAR_GRADIENT[c];
  }, [myColor]);

  const localPlayerDisplayName = useMemo(() => {
    const n = controlledPlayer?.name?.trim();
  ...
  useEffect(() => {
    if (effectiveStage !== prevStageRef.current) {
      ...
      // NoiseFieldOverlay disabled to avoid exhausting WebGL contexts.
    }
  }, [effectiveStage]);
  ...
  useEffect(() => {
    if (controlledPlayer && !predictedPos) {
      setPredictedPos({ x: controlledPlayer.x, y: controlledPlayer.y });
    }
  }, [controlledPlayer, predictedPos]);
  ...
  useEffect(() => {
    ...
    const handleKeyDown = (e: KeyboardEvent) => {
      ...
      if (direction) {
        ...
        if (!currentPlayer || !activeColor) return;
  ...
  }, [room, myColor, players, gridWidth, gridHeight, predictedPos, controlledPlayer, isSpectator, countdown, isGameOver, playSound]);
        ...
        >
          ...
          {isSoloMode ? (
            <>
              ...
              <div className="relative z-10 flex min-h-10 w-full shrink-0 flex-nowrap items-center justify-end gap-x-2 border-t border-white/10 px-3 py-2">
      ...
      >
        ...
        <ParticleFloor key={`floor-${gridWidth}-${gridHeight}`} gridWidth={gridWidth} gridHeight={gridHeight} spacing={SPACING} nodeStates={nodeStates} rippleTrigger={rippleTrigger} />
            ...
            {Array.from(players.values()).map((player, index) => {
              const isMe = player.sessionId === room?.sessionId || player.color === myColor;
